...
let
  ...
  # ---------------------------------------------------------------------------
  buildMeta = builtins.fromJSON (builtins.readFile ../../build-meta/triton-tensorrt-backend.json);
  buildVersion = buildMeta.build_version;
  pname = "triton-tensorrt-backend";

  trt = import ./tensorrt-cuda.nix {};

  # ---------------------------------------------------------------------------
  # Pre-fetch sources (4 total — backend + core/common/backend shared repos)
  # ---------------------------------------------------------------------------

  tensorrtBackendSrc = fetchFromGitHub {
    owner = "triton-inference-server";
    repo = "tensorrt_backend";
    rev = tag;
    hash = "sha256-cTjgFayo1ZnxDpEa5I7R0etGtobJnNaODEwcBy2zwO8=";
  };
...
gcc14Stdenv.mkDerivation {
  ...
  cmakeFlags = [
    ...
    "-DTRITON_TENSORRT_INCLUDE_PATHS=${trt.include}/include"
    "-DTRITON_TENSORRT_LIB_PATHS=${trt.lib}/lib"

    "-DCUDAToolkit_ROOT=${cudaPackages.cudatoolkit}"
  ];

  preConfigure = ''
    # Patch /etc/os-release in backend source (doesn't exist in Nix sandbox)
...
MARKER
  ...
  meta = with lib; {
    description = "TensorRT backend for NVIDIA Triton Inference Server";
    homepage = "https://github.com/triton-inference-server/tensorrt_backend";
